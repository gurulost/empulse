...
//                        ->where("email", "!=", $head);
    ...
    static public function send_letter($email, $name, $subject, $content) {
        try {
    ...
    static public function ifUserExist($email, $name) {
        $user = User::where('email', $email)->first();
        $result = false;

        if($user) {
            if(\Session::get('non-insert')) {
                $count = \Session::get('non-insert');
                $count += 1;
            } else {
                $count = 1;
            }
            \Session::put('non-insert', $count);
            $result = true;
        }

        if(\Session::get('insert')) {
            $count = \Session::get('insert');
            $count += 1;
        } else {
            $count = 1;
        }
        \Session::put('insert', $count);

        return $result;
    }

    static public function add_worker($name, $email, $password, $status, $department) {
        ...
        $ifUserExist = self::ifUserExist($email, $name);
    ...
    static public function add_worker_teamlead($name, $email, $teamlead) {
        $companyId = \Auth::user()->company_id;
        $company = \Auth::user()->company_title;
        $tariff = \Auth::user()->tariff;
        $link = env('LOGIN_URL');
