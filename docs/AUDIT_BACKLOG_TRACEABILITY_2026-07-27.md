...
| EMP-112 | Verified local; external gate | Respondent labels, untouched-slider handling, programmatic scale/error semantics, exact conditional/group error focus, autosave/progress live state, active-page/completion focus, reduced motion, 44px primary controls, and mobile layout are covered by component tests. A real PostgreSQL/web/worker Chromium journey passes actual-survey axe, keyboard privacy/error/slider/page behavior, five representative widths, and empty console/page errors. Fonts/icons are self-hosted and current templates are contract-tested against runtime CDN dependencies. Independent keyboard/screen-reader review remains. |
...
| EMP-505 | Verified local; external gate | Local PostgreSQL k6 readiness/login and 500-person analytics evidence pass. A source-bound 500-person full-wave rehearsal creates assignments/jobs in 1.686 seconds, remains unchanged when replayed, and restores 500 intentionally removed synthetic queue jobs without duplicates. A 500-row governed-roster rehearsal proves encrypted stage, parse, preview, atomic commit, exact roster/invitation queue counts, and same-file/commit replay idempotency. A paired-process race proves one same-revision autosave winner/one conflict and one final-submission winner/one conflict with exactly one 62-answer response and usage event. Approved-scale shared-service/provider execution remains. |
...
| EMP-509 | Verified local; external gate | [`RELEASE_AND_ROLLBACK_POLICY.md`](RELEASE_AND_ROLLBACK_POLICY.md) and deployment runbook define saved artifact identity, migration gates, feature controls, canary, rollback, and incident stop rules. Production now requires exact `APP_RELEASE_SHA` and environment identity; `app:verify-deployment` checks that an external HTTPS target serves those identities with fresh process readiness and security headers while explicitly withholding production sign-off. Selected-provider staging/canary/rollback remains. |
