...
| EMP-502 | Verified local; external gate | Pint, scoped no-baseline PHPStan, Composer/npm audit, lint, license inventory, current/proposed secret scans, and full-history scan are permanent gates. The old mail token must be revoked and the rehearsed history rewrite explicitly authorized. |
...
| EMP-505 | Verified local; external gate | Local PostgreSQL k6 readiness/login evidence, concurrency regressions, queue/mail scenarios, budgets, and 500-person roster/analytics scenarios are specified; approved-scale provider execution remains. |
