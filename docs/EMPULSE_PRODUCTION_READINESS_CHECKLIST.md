...
- Last Updated: 2026-07-28T15:41:00-04:00
- Canonical GitHub Branch: `main`
- Last Fully Validated Canonical Main Snapshot: `3e311fd91142003a62593a51e894df14199c386c`
...
- [ ] Q-203 [status:in_progress] EMP-112 and EMP-215: accessible respondent UX and production-scale analytics.
  - Evidence: untouched sliders, programmatic scale/error descriptions, conditional-control error targeting, progress/autosave live regions, active-state/page/completion focus, keyboard validation recovery, reduced motion, 44px primary touch targets, and narrow navigation are implemented. A real PostgreSQL/web/worker Chromium gate passes axe on the actual survey, keyboard entry/error/slider/page flow, empty browser/page-error consoles, and overflow/control bounds at 375, 390, 768, 1440, and 1920 pixels. A clean-source PostgreSQL 14 rehearsal measured eligible analytics for 411 valid responses from 508 invitations at p95 1,087.80 ms, with the selected-wave EXPLAIN query at 2.230 ms and the companion unfiltered latest-answer query at 7.666 ms. Provider staging-scale proof and independent keyboard/screen-reader review remain.
...
- [x] Q-500 [status:verified] EMP-501–504: PostgreSQL/process CI, static/security gates, Vue tests, and full browser journeys.
  - Evidence: canonical `main` snapshot `3e311fd91142003a62593a51e894df14199c386c` passed GitHub Actions run `30392355036`. Its PostgreSQL 16 product job passed migration/seed, 219 tests/1,302 assertions, dependency audits, caches, formatting/static analysis, eight frontend tests, the 200-module build, and all 17 real-process browser/accessibility/governance/product-loop journeys. Its secret job passed the strict proposed-change scan and CI-pinned Gitleaks 8.24.3 policy: the unignored full-history baseline is exactly the three owner-approved fingerprints, the exact-fingerprint scan passes, and an unrecognized synthetic finding fails.
- [ ] Q-501 [status:in_progress] EMP-505–507: load/capacity, observability/SLOs, backup/restore and integrity drills.
  - Evidence: health and heartbeat checks, SLO/capacity runbooks, k6 scenario, and PostgreSQL restore/audit-chain drill exist. Production configuration now requires process-heartbeat enforcement, a bounded heartbeat age, immutable release SHA, and stable environment identity. The external verifier fails on stale worker/scheduler state or served-build mismatch. The source-bound Pulse rehearsal passes at 508 invitations, 411 completions, 25,482 answers, p95 1,087.80 ms, and zero bounded duplicate/cross-tenant/mapping findings. A clean 500-row governed-roster run proves encrypted stage, parse/preview, exact atomic commit counts, and replay idempotency. A clean 500-person full-wave run creates assignments/jobs in 1,685.88 ms; identical job replay leaves all assignment, queue, and usage totals unchanged; report-first recovery restores 500 intentionally removed synthetic jobs with no duplicate or elevated dispatch counts. A paired-process race preserves one same-revision autosave and commits exactly one 62-answer final response plus usage event. Shared-service/provider queue-age, mail, worker-supervisor, alert, backup, and recovery drills remain.
- [ ] Q-502 [status:in_progress] EMP-508–510: independent assurance, release/rollback controls, and living runbooks/contracts.
  - Evidence: living product/architecture/privacy/methodology/release/rollback/DR documents and automated axe gate exist. Clean implementation `3dcf75a8bb702d05c28ad855a6a28aec6ab1f71a` adds an immutable served-release/environment contract plus `app:verify-deployment`, which verifies an external HTTPS origin, exact SHA/environment on liveness and readiness, fresh worker/scheduler state, and required security headers while always reporting `production_signoff=false`. Obsolete checklists and the old deploy handoff are explicitly archived and removed from the current documentation map; current roster/onboarding copy no longer promises a disabled importer; repository documentation contains no machine-specific absolute home-directory links. Independent reviews and actual provider staging evidence remain.
...
- [ ] P-009 [status:in_progress] Establish the complete permanent production assurance system.
  - Addresses: F-013
  - Evidence: permanent CI, checklist validation, current release/rollback/DR/observability/capacity runbooks, historical-document isolation, automated browser/accessibility coverage, immutable served-build identity, external deployment-surface verification, source-bound PostgreSQL analytics/integrity and 500-person dispatch/recovery rehearsals, unique invitation jobs, and scheduled report-first recovery are implemented. Provider drills and independent human review remain.
...
- [x] V-004 [status:verified] `npm run build`
  - Evidence: Vite 8.1.5 production build passes with 200 transformed modules, including self-hosted OFL fonts and MIT Bootstrap Icons.
...
- [x] V-007 [status:verified] Static analysis, dependency, secret, security, and license gates.
  - Evidence: full-tree Pint is clean; scoped critical-path Larastan passes without a baseline; Composer and npm report zero advisories; PHP and direct JavaScript licenses were inventoried with no unknown direct licenses. Direct runtime additions are MIT Bootstrap Icons and OFL-1.1 DM Sans, Outfit, and Inter; the unused PHPMailer dependency was removed. Current-source and proposed-change Gitleaks scans are strict and clean. The mail administrator confirmed that the historical Sendinblue/Brevo credential was revoked and deactivated; the owner declined history rewriting and approved only the three exact root `.gitleaksignore` fingerprints. The CI-pinned 8.24.3 policy check proves the unignored baseline is exactly those three findings, the approved full-history scan passes, and a new unrecognized finding fails.
- [ ] V-008 [status:in_progress] Accessibility component/browser and manual review.
  - Evidence: clean implementation `070447da2ec42379da52509b9b6b9d860fbbb7f0` adds a Tier 2 original-design respondent brief plus component contracts for slider/error/group semantics. Axe reports no serious/critical WCAG A/AA violations on public, login, manager, action, employee, respondent-promise, and actual survey states. The real Chromium journey proves keyboard privacy entry, first-error focus/recovery, slider operation, page-heading focus, reduced motion, saved-state visibility, empty consoles, 44px primary controls, and no horizontal overflow at five representative widths. Independent keyboard/screen-reader review remains.
- [ ] V-009 [status:in_progress] Load, concurrency, queue, scheduler, mail, and Stripe test-mode evidence.
  - Evidence: local PostgreSQL k6 smoke passes 7,504 iterations/22,512 requests with zero failures, p95 182.82 ms, and p99 191.15 ms at 20 VUs. Source-bound local evidence also passes a 500-row roster stage/parse/commit in 12.21/150.3/4,820.07 ms with exact idempotent counts, 508-invite analytics at p95 1,087.80 ms, and 500-person full-wave dispatch in 1,685.88 ms. Wave-job replay and report-first recovery remain duplicate-safe. A paired-process PostgreSQL race produces one autosave success/one conflict and one final submission/one conflict, with exactly one 62-answer response and usage event. Provider-backed queue-age, mail, Stripe, durable shared-service, alert, and worker-failure drills remain.
- [ ] V-010 [status:in_progress] Backup/restore, migration rollback, readiness, and incident drills.
  - Evidence: final-state PostgreSQL custom backup restored into a new database; six critical row counts match (1 company, 9 users, 10 assignments, 8 responses, 496 answers, 17 audit events) and restored platform/company audit chains verify. The governed-roster migration and the final four evidence/advisor/publication/billing-history migrations roll back and reapply cleanly. Production config now refuses disabled/stale heartbeat policy or missing release/environment identity, and `app:verify-deployment` rejects wrong/stale externally served runtime surfaces. Provider backup/PITR, full application rollback, and alert drills remain.
...
- [ ] R-002 [status:open] Provider-specific runtime proof requires a selected hosting, mail, and observability environment.
  - Rationale: repository readiness can be proven provider-neutrally; live-provider behavior cannot. `docs/LAUNCH_DECISIONS_AND_EXTERNAL_REVIEW_HANDOFF.md` now names the provider decision inputs and exact staging evidence required without claiming that a provider has been selected.
  - Owner: release owner
...
- [x] R-004 [status:accepted_risk] Git history contains a revoked Sendinblue/Brevo token and two generic-key detections in removed legacy attachments.
  - Rationale: the repository owner and mail administrator confirmed the historical credential was revoked and deactivated, declined a destructive history rewrite, and chose to preserve existing commits. The dead credential remains visible in old commits, clones, and cached historical views; it has not been removed. The accepted exception is limited to the three exact fingerprints in the root `.gitleaksignore`, while all current-source, proposed-change, and unrecognized-history findings continue to fail closed.
  - Owner: repository owner and WorkFit mail administrator
  - Follow-up trigger/date: reassess if revocation status is disputed, a provider or regulator requires physical history removal, a new finding appears, or the risk owner withdraws acceptance.
  - Control: `.github/verify-gitleaks-history-policy.sh` pins the exact approved set, checks the unignored baseline, and runs a synthetic negative control with Gitleaks 8.24.3. `docs/GIT_HISTORY_SECRET_REMEDIATION_RUNBOOK.md` records the declined rewrite and retains its rehearsal only as a future contingency.
...
- 2026-07-28T12:44:06-04:00: Recorded owner-confirmed revocation/deactivation of the historical Sendinblue/Brevo credential and the explicit decision to preserve Git history. Marked R-004 as an accountable accepted risk: the dead credential remains visible in old commits. Added only the three exact fingerprints to the root `.gitleaksignore`; CI-pinned Gitleaks 8.24.3 proves the unignored baseline is exactly that set, the approved full-history scan passes, and a newly generated unrecognized finding still fails. The rehearsed force-push rewrite is declined and retained only as a future contingency.
- 2026-07-28T14:59:58-04:00: Bound future deployed artifacts to an exact Git SHA and stable environment identity, made production process-heartbeat enforcement non-optional and bounded, exposed the configured identity consistently through health payloads and `X-Empulse-Release`, and added `app:verify-deployment` to fail closed on non-HTTPS origins, redirects, identity mismatch, stale worker/scheduler state, or missing response security headers. The verifier explicitly reports `production_signoff=false`; no live target has been tested or claimed.
- 2026-07-28T15:23:47-04:00: Completed the automatable Tier 2 respondent accessibility gate at clean implementation `070447da2ec42379da52509b9b6b9d860fbbb7f0`. The real survey now has deterministic active-state/error/completion focus, programmatic scale/error descriptions, conditional error targeting, reduced motion, mobile navigation, and touch sizing. The rendered PostgreSQL/web/worker Chromium gate passes actual-survey axe, keyboard recovery/navigation, five widths, and clean console checks. That console gate exposed the layouts loading assets their CSP blocked; fonts/icons are now self-hosted through Vite, unused Chart CDNs and legacy Toastr CDNs were removed, and a source contract prevents their return. Independent human keyboard/screen-reader review remains open.
- 2026-07-28T15:41:00-04:00: Canonical `main` snapshot `3e311fd91142003a62593a51e894df14199c386c` passed GitHub Actions run `30392355036` end to end after regenerating the lockfile with the runner's npm 10.9.8 dependency graph. Both product and secret jobs are green. Added a launch-decision/external-review handoff so owner, reviewer, and provider evidence can be gathered without representing working defaults or repository tests as external approval.
