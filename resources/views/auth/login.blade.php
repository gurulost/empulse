...
@section('content')
            ...
            <form method="POST" action="{{ route('login') }}" class="login-form-content">
                    ...
                    <div class="log-form-flex">
                        ...
                        <div class="log-form-enter">
                            @if (isset($_GET['testtt']))
                                <button id="submitBtn" title='Fulling email, password and pass the CAPTCHA' type="submit" class="log-f-enter-btn"><span class="log-f-enter-text">Enter</span></button>
                            @else
                                <button id="submitBtn" onclick="clearStorage()" title='Fulling email, password and pass the CAPTCHA' type="submit" class="log-f-enter-btn" disabled ><span class="log-f-enter-text">Enter</span></button>
                            @endif
                        </div>
                    </div>
                </div>
                @if (!isset($_GET['testtt']))
                    <div class="log-form-captcha">
                        {!! NoCaptcha::renderJs() !!}
                        {!! NoCaptcha::display(['data-callback' => 'onSubmit', 'data-expired-callback' => 'onExpire']) !!}
                    </div>
                @endif
    ...
    <script>
        ...
        function enableSubmitButton() {
            ...
            document.getElementById('submitBtn').style.background = "#F1C82D";
        }
